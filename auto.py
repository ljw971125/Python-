...
def auto_save_hour():
    ...
    # 파일 저장 모듈
    def savefile():
        if(os.path.isfile('save_data.txt')): # 경로상에 save_data.txt 이름의 파일이 있을 경우
            ...
            f=open('save_data.txt','w',encoding='utf-8') # 파일 write 방식으로 열기(write방식으로 없을 경우 자동 생성)
            for i in fixed_list: 
                f.write(i+'\n') # '\n' 으로 줄을 띄우면서 파일에 씁니다
        f.close() # 파일 닫기
        
    def get_rank():  # 인기 검색어 상위 20개씩 뽑기 위한 함수
